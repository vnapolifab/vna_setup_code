...
"""
Analyzes and plots spin-wave measurements.

Args:
    freq (np.ndarray): Frequency data (Hz).
    fields (np.ndarray): Magnetic field values (mT).
    amplitudesX (np.ndarray): Amplitude data for 4 traces.
    phasesX (np.ndarray): Phase data for 4 traces.
    measurement_path (str): Path to save plot outputs.
    Ports (str): Defines the type of ports (e.g., '12' for S11, S21, etc.).
    ref_n (int): Index for reference point.
    show_plots (bool): Whether to display and save plots.
"""


def analysisSW(freq: np.ndarray, fields: np.ndarray, amplitudes1: np.ndarray, phases1: np.ndarray, amplitudes2: np.ndarray, phases2: np.ndarray, amplitudes3: np.ndarray, phases3: np.ndarray, amplitudes4: np.ndarray, phases4: np.ndarray, measurement_path: str, Ports: str, ref_n = 0, show_plots=True):


    # Number of traces and data points (frequency points)
    n_traces = len(amplitudes1[:,0])
    n_points = len(freq)

    # Initialize arrays for processed data (real, imaginary, and complex values)
    ...
    # Repeat for traces 2, 3, and 4 (same structure as for trace 1)
    ...
    # Extract reference amplitudes and phases for each trace
    ...
    # Signal processing for each trace (subtraction of background and unwrapping phase)
    for i in range(n_traces):
        ...
        # Subtract reference values for each trace
        #Trace 1
    ...
    # Convert amplitudes to decibels (dB)
    ...
    # Determine the correct label based on the port configuration (e.g., '12' corresponds to S11, S21, S12 and S22)
    if (Ports == '12'):
        S1 = 'S11'
        S2 = 'S21'
        S3 = 'S12'
    ...
    # Plot Imaginary, Real, Amplitude (linear), Phase (unwrapped), and Amplitude (dB) for each S-parameter
    # Multiple plots are generated here using Matplotlib, with labels for the different magnetic field strengths used
    # The plots are saved to the measurement folder in PNG format
    if show_plots:
